...
function extractWidth(sku) {
  ...
  // (e.g. B9L = 9" wide base). Without this, narrow 9" SKUs return null.
  if (/^9(?!\d)/.test(numPart)) return 9
...
export function parseDimensions(sku, section) {
  ...
  // (e.g. B9L = 9" wide). Without this, dimStr="9" yields zero pairs below.
  if (/^9(?!\d)/.test(dimStr)) {
    pairs.push(9)
    var remainder = dimStr.slice(1)
