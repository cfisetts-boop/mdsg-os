import { APPLIANCE_RE } from './skuRules'
/**
...
const ZERO_HARDWARE_PREFIXES = ['F3', 'F4', 'F5', 'F6', 'TRP', 'BRP', 'FS', 'STVAL', 'TKPW', 'TKC', 'OCM', 'WF', 'TF', 'BEP', 'REP', 'TREP', 'VEP', 'TEP', 'EPB', 'TK8', 'TK', 'TSK', 'SCM', 'SMC']
...
// ── ADA / accessible height detection ─────────────────────────────────────────
// This project's SKUs mark accessible cabinets with a literal "-ADA" or "-HC"
// suffix on the SKU itself (e.g. DB18-3-HC, B9L-ADA) — NOT the "-32.5" suffix
// used on other projects. Detect both conventions so height always reflects
// the true ADA status of the individual cabinet, not just the unit type.
function isAccessibleSku(sku) {
  const u = (sku || '').toUpperCase()
  return /(-ADA\b|-HC\b|-HCA\b|32\.5)/.test(u)
}

// Tall vanity marker — a trailing "T" (optionally followed by a hinge letter)
// after the drawer-count or width, e.g. VDB18-4T, VDB15-4-T, VSB36T.
function isTallVanitySku(sku) {
  const u = (sku || '').toUpperCase()
  return /(\d|-)T([LR])?$/.test(u)
}

// Strip trailing accessibility/handling tags (-HC, -ADA, -RF, -HD, -FH) so an
// embedded dash-number drawer/hardware count underneath them can still be
// found, e.g. "DB18-3-HC" → "DB18-3" → count is 3, "-HC" only affects height.
function stripSuffixTags(sku) {
  let s = sku
  let changed = true
  while (changed) {
    changed = false
    const next = s.replace(/-(HC|HCA|ADA|RF|HD|FH)$/i, '')
...
function extractWidth(sku) {
  ...
  // Single-digit width fallback — only "9" is a valid standalone cabinet width
  // (e.g. B9L = 9" wide base, or W936L = 9w x 36h slim wall cabinet where the
  // leading 9 isn't followed by a second width digit, just straight into the
  // height digits — 93 and 936 are NOT real cabinet widths). Must run BEFORE
  // the 3-digit fallback below, or "936..." gets misread as a 936" cabinet.
  if (/^9/.test(numPart)) return 9
...
export function getSection(sku) {
  ...
  // ── Toe Kick — includes bare "TK" used on some projects, not just "TK8" ──
  if (/^(TKPW|TKC|TK8|TK|TSK)/.test(u))                  return 'Toe Kick'
  // ── End Panel (REP=rear, TREP=tall rear, VEP=vanity end panels) ──
  if (/^(PLYS|EPT|AD21|REP|TREP|VEP|TEP)/.test(u))       return 'End Panel'
  if (/^(FS\d|BRP|TRP)/.test(u))                          return 'End Panel'   // furniture skins & return panels
  if (/^STVAL/.test(u))                                   return 'Filler'      // starter valance
  // ── Scribe molding (SCM75 etc; SMC = common typo) ──
  if (/^(SCM|SMC)/.test(u))                              return 'Scribe'
  // ── Filler / OCM ──
  ...
  // ── Wall ──
  if (/^DW\d{4}/.test(u))                               return 'Wall'  // Diagonal Wall corner cabinet
  if (/^(BW|CW|WBC|WHL|WO|MW)/.test(u))                 return 'Wall'
...
export function calculateHardware(sku) {
  ...
  // 3.5. Sink Base — always 2 hardware regardless of width or suffix tags
  //      (e.g. SB33, SB33-ADA-RF, SB30-HC-RF all = 2)
  if (/^SB/.test(upperBase)) return { hardware: 2, specialCase: false, note: 'Sink Base — always 2' }

  // 4. Dash-number suffix = drawer/door count (ref guide §4)
  //    DB15-3 = 3, VDB12-3 = 3. Max plausible = 6 (CVSDB48 special case = 6).
  //    Strip trailing -HC/-ADA/-RF/etc first so a count tucked underneath
  //    one of those tags is still found, e.g. DB18-3-HC → DB18-3 → count 3.
  const dashSearchBase = stripSuffixTags(upperBase)
  const dashMatch = dashSearchBase.match(/-(\d+)[A-Z]?$/)
  if (dashMatch) {
  ...
  // Lazy Susan (bare "LS" prefix, not just blind-base "BLS"): always 1
  if (/^LS/.test(upperBase))  return { hardware: 1, specialCase: false, note: 'Lazy Susan (LS) — always 1' }
  // Utility Cabinet: always 2
  if (/^UC/.test(upperBase))  return { hardware: 2, specialCase: false, note: 'Utility Cabinet (UC) — always 2' }
  ...
  // Diagonal Wall corner cabinet (DW2430R etc): single door — 1
  if (/^DW\d{4}/.test(upperBase)) return { hardware: 1, specialCase: false, note: 'Diagonal Wall — always 1' }
  ...
  // Wall (including MW = microwave wall cabinet): ≤21" = 1, >21" = 2
  if (/^(W|MW)/.test(upperBase)) {
...
export function parseDimensions(sku, section) {
  ...
  const dimStr = u.replace(/^[A-Z-]+/, '').replace(/X\d+.*$/, '').replace(/[A-Z].*$/, '').replace(/^0+/, '')  // strip leading zeros (BMR02418 → 2418 = 24w 18h)
  const pairs = []
  // Single-digit width fallback — "9" is the only valid standalone width.
  // Two cases: "9" alone (e.g. B9L-ADA), or "9" followed straight into the
  // height digits with no second width digit (e.g. W936L = 9w x 36h, NOT
  // a 93"-wide cabinet — 93 isn't a real cabinet width).
  const hasTwoDigits = dimStr.length >= 2 && /^\d{2}/.test(dimStr)
  const firstTwo = hasTwoDigits ? parseInt(dimStr.substring(0, 2), 10) : null
  if (/^9/.test(dimStr) && !(hasTwoDigits && VALID_WIDTHS.has(firstTwo))) {
    pairs.push(9)
    const remainder = dimStr.slice(1)
    for (let i = 0; i + 1 < remainder.length; i += 2) {
      const pair = remainder.substring(i, i + 2)
  ...
  const w = pairs[0]
  const height = pairs.length >= 2 ? `${pairs[1]}"` : standardHeight(section, sku)
...
function standardHeight(section, sku) {
  const isAccessible = isAccessibleSku(sku)
  const isTallVanity = isTallVanitySku(sku)

  if (section === 'Base' || section === 'Base - Drawer' || section === 'Base - Sink') {
    return isAccessible ? '32½"' : '34½"'
  }
  if (section === 'Vanity Base' || section === 'Vanity Base - Sink' || section === 'Corner Vanity Base') {
    if (isTallVanity) return '34½"'
    if (isAccessible)  return '32½"'
    return '31½"'
  }
